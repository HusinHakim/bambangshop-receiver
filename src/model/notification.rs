...
impl Display for Notification {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        ...
        } else if self.status.to_uppercase().eq("SHIPPED") {
            return write!(f,
                "Hello {}! Be informed that our {} product called {} already sold out...",
                self.subscriber_name, self.product_type.to_lowercase(), self.product_title);
        } else {
            return write!(f,
                "Hello {}, let's try our {} product: {}, grab it out before the stock run out! Check it out: {}",
                self.subscriber_name, self.product_type.to_lowercase(), self.product_title, self.product_url);
        }
    }
} 